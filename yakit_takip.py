...
class SiparisTakipUygulamasi:
    def __init__(self, root):
        ...
        # Font ayarları (daha küçük boyutlarda)
        self.baslik_font = tkfont.Font(family='Segoe UI', size=12, weight='bold')
        self.normal_font = tkfont.Font(family='Segoe UI', size=10)
        self.buton_font = tkfont.Font(family='Segoe UI', size=10, weight='bold')
        self.tablo_font = tkfont.Font(family='Segoe UI', size=9)
    ...
    def veritabani_olustur(self):
        ...
        # Veritabanı klasörü oluştur (yoksa)
        veritabani_klasoru = os.path.join(uygulama_dizini, "veritabani")
        if not os.path.exists(veritabani_klasoru):
            os.makedirs(veritabani_klasoru)
        
        self.veritabani_yolu = os.path.join(veritabani_klasoru, "siparisler.db")
    ...
    def arayuz_olustur(self):
        ...
        # Ana çerçeveler
        self.ust_cerceve = tk.Frame(self.root, bg=self.renkler['cerceve'], padx=10, pady=10)
        self.ust_cerceve.pack(fill=tk.X)
        
        self.orta_cerceve = tk.Frame(self.root, bg=self.renkler['arkaplan'], padx=10, pady=5)
        self.orta_cerceve.pack(fill=tk.BOTH, expand=True)
        
        self.alt_cerceve = tk.Frame(self.root, bg=self.renkler['cerceve'], padx=10, pady=10)
        ...
        tk.Label(self.ust_cerceve, text="Sipariş Takip Sistemi", 
                font=self.baslik_font, bg=self.renkler['cerceve'], fg=self.renkler['baslik']).grid(row=0, column=0, columnspan=3, pady=(0,5), sticky=tk.W)
        ...
        self.filtre_secim = ttk.Combobox(self.ust_cerceve, values=["Tümü", "Sipariş gelmedi", "Kısmi teslimat", "Tamamı teslim alındı", "Gecikmiş"], 
                                        width=18, font=self.normal_font)
        ...
        self.baslangic_tarih = tk.Entry(self.ust_cerceve, width=10, font=self.normal_font)
        ...
        self.bitis_tarih = tk.Entry(self.ust_cerceve, width=10, font=self.normal_font)
        ...
        self.ara_giris = tk.Entry(self.ust_cerceve, width=25, font=self.normal_font)
        ...
        # Sütun genişlikleri
        self.tablo.column("siparis_no", width=70, anchor=tk.CENTER)
        self.tablo.column("malzeme", width=150, anchor=tk.W)
        self.tablo.column("miktar_birim", width=80, anchor=tk.CENTER)
        self.tablo.column("tedarikci", width=120, anchor=tk.W)
        self.tablo.column("siparis_tarihi", width=90, anchor=tk.CENTER)
        self.tablo.column("teslim_tarihi", width=90, anchor=tk.CENTER)
        self.tablo.column("siparisi_veren", width=100, anchor=tk.CENTER)
        self.tablo.column("durum", width=130, anchor=tk.CENTER)
    ...
    def siparis_ekle_penceresi(self):
        ...
        self.ekle_penceresi.geometry("450x500")
        self.ekle_penceresi.resizable(False, False)
        
        # Form bileşenleri
        tk.Label(self.ekle_penceresi, text="Sipariş Edilen Malzeme:", 
                font=self.normal_font).grid(row=0, column=0, padx=10, pady=5, sticky=tk.W)
        self.malzeme_giris = tk.Entry(self.ekle_penceresi, width=30, font=self.normal_font)
        self.malzeme_giris.grid(row=0, column=1, padx=10, pady=5)
        self.malzeme_giris.focus_set()
        
        tk.Label(self.ekle_penceresi, text="Miktar:", 
                font=self.normal_font).grid(row=1, column=0, padx=10, pady=5, sticky=tk.W)
        
        miktar_cerceve = tk.Frame(self.ekle_penceresi)
        miktar_cerceve.grid(row=1, column=1, padx=10, pady=5, sticky=tk.W)
        ...
        tk.Label(self.ekle_penceresi, text="Tedarikçi:", 
                font=self.normal_font).grid(row=2, column=0, padx=10, pady=5, sticky=tk.W)
        ...
        self.tedarikci_giris.grid(row=2, column=1, padx=10, pady=5)
        
        tk.Label(self.ekle_penceresi, text="Sipariş Tarihi:", 
                font=self.normal_font).grid(row=3, column=0, padx=10, pady=5, sticky=tk.W)
        self.siparis_tarihi_giris = tk.Entry(self.ekle_penceresi, width=30, font=self.normal_font)
        self.siparis_tarihi_giris.grid(row=3, column=1, padx=10, pady=5)
        ...
        tk.Label(self.ekle_penceresi, text="Teslim Edilmesi İstenen Tarih:", 
                font=self.normal_font).grid(row=4, column=0, padx=10, pady=5, sticky=tk.W)
        self.teslim_tarihi_giris = tk.Entry(self.ekle_penceresi, width=30, font=self.normal_font)
        self.teslim_tarihi_giris.grid(row=4, column=1, padx=10, pady=5)
        self.teslim_tarihi_giris.insert(0, datetime.now().strftime("%d.%m.%Y"))
        
        tk.Label(self.ekle_penceresi, text="Siparişi Veren:", 
                font=self.normal_font).grid(row=5, column=0, padx=10, pady=5, sticky=tk.W)
        ...
        self.siparisi_veren_giris.grid(row=5, column=1, padx=10, pady=5)
        
        tk.Label(self.ekle_penceresi, text="Açıklama:", 
                font=self.normal_font).grid(row=6, column=0, padx=10, pady=5, sticky=tk.W)
        self.aciklama_giris = tk.Text(self.ekle_penceresi, width=30, height=4, font=self.normal_font)
        self.aciklama_giris.grid(row=6, column=1, padx=10, pady=5)
        
        # Kaydet butonu
        tk.Button(self.ekle_penceresi, text="Kaydet", command=self.siparis_kaydet, 
                 bg=self.renkler['buton'], fg='white', font=self.buton_font,
                 activebackground=self.renkler['buton_hover'], activeforeground='white',
                 padx=15).grid(row=7, column=1, pady=10, sticky=tk.E)
    ...
    def siparis_duzenle(self):
        ...
        self.duzenle_penceresi.geometry("450x500")
        ...
        self.malzeme_giris_duzenle.grid(row=0, column=1, padx=10, pady=5)
        ...
        self.tedarikci_giris_duzenle.grid(row=2, column=1, padx=10, pady=5)
        ...
        self.siparis_tarihi_giris_duzenle.grid(row=3, column=1, padx=10, pady=5)
        ...
        self.teslim_tarihi_giris_duzenle.grid(row=4, column=1, padx=10, pady=5)
        ...
        self.siparisi_veren_giris_duzenle.grid(row=5, column=1, padx=10, pady=5)
        self.siparisi_veren_giris_duzenle.set(siparis[7] if siparis[7] else "")
        
        tk.Label(self.duzenle_penceresi, text="Açıklama:", 
                font=self.normal_font).grid(row=6, column=0, padx=10, pady=5, sticky=tk.W)
        self.aciklama_giris_duzenle = tk.Text(self.duzenle_penceresi, width=30, height=4, font=self.normal_font)
        self.aciklama_giris_duzenle.grid(row=6, column=1, padx=10, pady=5)
    ...
    def durum_guncelle_penceresi(self):
        ...
        tk.Button(self.durum_penceresi, text="Güncelle", 
                 ...
                 bg=self.renkler['buton'], fg='white', font=self.buton_font,
                 activebackground=self.renkler['buton_hover'], activeforeground='white',
                 padx=15).grid(row=3, column=1, pady=15, sticky=tk.E)
        
        # Başlangıçta duruma göre alanları ayarla
        self.durum_secim_degisti(None)
    
    def durum_secim_degisti(self, event):
        """Durum seçimine göre ek alanları gösterir/gizler"""
        secilen_durum = self.durum_secim.get()
        
        if secilen_durum == "Kısmi teslimat":
        ...
        elif secilen_durum == "Tamamı teslim alındı" or secilen_durum == "Gecikmiş":
    ...
    def durum_guncelle(self, siparis_id):
        ...
        elif yeni_durum == "Tamamı teslim alındı" or yeni_durum == "Gecikmiş":
            teslim_tarihi = self.teslim_tarihi_giris.get()
        
        # Tarih kontrolü (Gecikmiş durumu için de teslim tarihi zorunlu)
        try:
        ...
        # Gecikmiş durumu için teslim tarihi kontrolü
        if yeni_durum == "Gecikmiş" and not teslim_tarihi:
            messagebox.showerror("Hata", "Gecikmiş durumu için teslim tarihi gereklidir!")
            return
    ...
    def rapor_al(self):
        ...
        rapor_penceresi.geometry("500x400")
    ...
    def yedek_al(self):
        ...
        if dosya_yolu:
            try:
                ...
                shutil.copy2(self.veritabani_yolu, dosya_yolu)
    ...
    def yedekten_geri_yukle(self):
        ...
        if dosya_yolu:
            ...
            try:
                ...
                shutil.copy2(dosya_yolu, self.veritabani_yolu)
    ...
    def kullanim_kilavuzu(self):
        ...
        kilavuz_penceresi.geometry("500x400")
    ...
    def hakkinda(self):
        ...
        Bu uygulama sipariş takibi yapmak için geliştirilmiştir.
